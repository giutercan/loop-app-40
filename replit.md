...
#### Discovery Export (Comprehensive PPT/PDF)
The Discovery Export generates branded Korn Ferry PowerPoint and PDF reports from the Export Discovery Report dialog in the Sales Workspace. The export dialog (`ExportOptionsDialog.tsx`) features a KF navy-branded header and 4 selectable sections (Strategic Theme, Market Intelligence, Client Interaction, Discovery & Analysis) with sub-section badges. Reports are generated client-side using `pptxgenjs` and `jspdf` in `client/src/lib/exportService.ts`. Both generators are async (to render SVG-based KF logos as PNG via canvas). PPT slides include: title slide with KF logo, company profile, strategic insights (paginated), theme analysis, recent news, competitive landscape, key executives, annual report, earnings call, meeting attendees, Green Sheet, discovery questions, story coaching, narrative canvas, discovery notes, value cases, probe research, project notes, data points, and external links. Every slide has a branded header (navy/teal gradient with "KORN FERRY" text) and footer ("Korn Ferry | Confidential" + page number + logo). PDF mirrors the same sections with branded headers, teal section dividers, and page footers on every page.
